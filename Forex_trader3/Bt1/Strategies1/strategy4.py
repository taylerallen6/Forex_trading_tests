...
class TestStrategy(bt.Strategy):
  ...
  def __init__(self):
    ...
    # self.sma_daily = btind.SMA(self.data1, period=self.period)
  ...
  def notify_order(self, order):
    ...
    if order.status in [order.Completed]:
      if order.isbuy():
        ...
        stop_loss_price = order.executed.price - (self.range * .4)
        self.close(exectype=bt.Order.Stop, price=stop_loss_price)
        # take_profit_price = order.executed.price + (self.range * .9)
    ...
    #   stop_loss_price = order.executed.price - (self.range * .25)
    #   self.close(exectype=bt.Order.Stop, price=stop_loss_price)
    #   take_profit_price = order.executed.price + (self.range * .75)
    #   self.close(exectype=bt.Order.Limit, price=take_profit_price)
  ...
  def next(self):
    ...
    if self.position:
      ...
      self.trade_days = time_diff.days
